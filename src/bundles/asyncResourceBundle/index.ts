...
export type IndexedResource<Resource extends ResourceWithId<IdProp>, IdProp extends string> = {
  [id: string]: Resource;
};

export type StateData<Resource extends {}, IdProp extends string> = Resource extends ResourceWithId<
  IdProp
>
...
export type RootStateWithBundle<Resource, EntityName extends string, IdProp extends string> = {
  [entity in EntityName]: State<Resource, IdProp>;
};
...
const createAsyncResourceBundle = <
  ...
  RootState extends RootStateWithBundle<Resource, EntityName, IdProp> = RootStateWithBundle<
    Resource,
    EntityName,
    IdProp
  >
...
>(
  ...
  const {
    indexById = false,
    selectLocalState = (state: RootState): State<Resource, IdProp> => state[entityName]
  } = options;
  ...
  return {
      ...
      switch (action.type) {
        ...
        case FETCH_SUCCESS: {
          return {
            ...
            data: !indexById
              ? action.payload.data
              : formatIncomingResourceData(state.data, action.payload.data, entityName),
            ...
            loadingIds: indexById
              ? removeStatusIds(state.loadingIds, getStatusIdsFromData(action.payload.data))
              : [],
        ...
        case FETCH_ERROR: {
          ...
          return {
            ...
            loadingIds: indexById ? removeStatusIds(state.loadingIds, action.payload.ids) : []
          };
        }
        case UPDATE_START: {
          return {
    ...
    selectors: createSelectors<Resource, IdProp, RootState>(selectLocalState)
  };
};

export { IPagination, globalLoadingIndicator, createAsyncResourceBundle };
